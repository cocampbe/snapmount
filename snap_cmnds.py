...
def unlink_and_terminate_snap(src_sid,dest_sid):
  ...
  src_sg_name = "{src_host}_{src_sid}".format(src_host=sids[src_sid],src_sid=src_sid).upper()
  dest_sg_name = "{dest_host}_{dest_sid}_snap".format(dest_host=host,dest_sid=dest_sid).upper()
  print "Unlinking snap {snap_name} from {src_sg_name}.".format(snap_name=snap_name,src_sg_name=src_sg_name)
  ...
  print "Terminating snap {snap_name}.".format(snap_name=snap_name)
...
def add_sg_to_mv(dest_sid):
  ...
  print "Checking if {dest_sg_name} is already a child of {parent_sg_name}.".format(dest_sg_name=dest_sg_name,parent_sg_name=parent_sg_name)
  ...
  if "IsChild" in grep_symsg_parent_info.communicate()[0]: 
    print " * {dest_sg_name} is already a child of SG {parent_sg_name}. Continuing...".format(dest_sg_name=dest_sg_name,parent_sg_name=parent_sg_name)
  else: 
    print " * {dest_sg_name} is not a child of {parent_sg_name}. Adding.".format(dest_sg_name=dest_sg_name,parent_sg_name=parent_sg_name)
    ...
    if symsg_add_status.returncode != 0: 
      print " * Error adding {dest_sg_name} to parent_sg_name. Exiting.".format(dest_sg_name=dest_sg_name,parent_sg_name=parent_sg_name)
      exit(1)
...
def remove_sg_from_mv(dest_sid):
  ...
  parent_sg_name = "{dest_host}_SG".format(dest_host=host).upper()
  print "Removing {sg_name} from parent {parent_sg_name}.".format(sg_name=sg_name,parent_sg_name=parent_sg_name)
  ...
  if symsg_remove.returncode != 0:
    if "No Storage groups" in symsg_remove_err.strip():
      ...
      print " * Error removing {sg_name} from {parent_sg_name}. Exiting.".format(sg_name=sg_name,parent_sg_name=parent_sg_name)
      exit(1)
...
def disk_cleanup(dest_sid):
  ...
  if "Not found" in disk_dev_file:
    ...
    print "Removing disk {disk_dev_file}.".format(disk_dev_file=disk_dev_file)
...
def import_activate_vg(dest_sid):
  ...
  if vgs_returncode == 0:
    print " * Volume group {vg} is already imported. Continuing...".format(vg=vg_basename) 
  else:
